    ...
    return (
        <div className="relative bg-white rounded-2xl px-4 py-6 mt-8 mx-56 overflow-hidden">
            <div className="flex items-center space-x-4">
                <p className="font-bold text-textColor text-2xl">Cabinet assembly</p>
                {
                        ...
                        >
                            {item.step < currentStep ? (
                                ...
                                <p className={`flex items-center justify-center w-7 h-7 rounded-full font-semibold 
                                    ${item.step === currentStep ? 'bg-white text-primary' : 'bg-secondary text-white'}`}>
                                    {item.number}
                                </p>
                            )}
                            <p className={`text-sm font-light 
                                ${item.step === currentStep ? 'text-white' : 'text-secondary'}`}>
                                {item.text}
                            </p>
                        </div>
                    ))
            ...
            <div className="absolute bottom-0 left-0 w-full h-1.5 bg-white rounded-2xl">
                <div className="h-full bg-primary progress-bar" style={{ width: `${progressPercentage}%` }}></div>
            </div>
        </div>
    );
}